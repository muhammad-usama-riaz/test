...
class BookingRepository extends BaseRepository
{
		...
		if ($cuser) {
			...
			if ($jobs) {
				foreach ($jobs as $jobitem) {
					$this->sortJobs($jobitem, $emergencyJobs, $normalJobs, $user_id);
				}
	...
	private function sortJobs($jobitem, &$emergencyJobs, &$normalJobs, $user_id)
	{
		if ($jobitem->immediate == 'yes') {
			$emergencyJobs[] = $jobitem;
		} else {
			$normalJobs[] = $this->processNormalJob($jobitem, $user_id);
		}
	}
	
	private function processNormalJob($jobitem, $user_id)
	{
	...
	public function getUsersJobsHistory($user_id, Request $request)
	{
		...
		if ($cuser) {
			if ($cuser->is('customer')) {
				return $this->getCustomerJobsHistory($cuser, $page);
			} elseif ($cuser->is('translator')) {
	...
	private function getCustomerJobsHistory($cuser, $page)
	{
		...
		return ['emergencyJobs' => [], 'noramlJobs' => [], 'jobs' => $jobs, 'cuser' => $cuser, 'usertype' => 'customer', 'numpages' => $jobs->lastPage(), 'pagenum' => $page];
	}
	...
	private function getTranslatorJobsHistory($cuser, $page)
	{
		...
		return ['emergencyJobs' => [], 'noramlJobs' => $jobs_ids, 'jobs' => $jobs_ids, 'cuser' => $cuser, 'usertype' => 'translator', 'numpages' => $numpages, 'pagenum' => $page];
	}
	...
	public function store($user, $data)
	{
		...
		if (!isset($data['from_language_id']) || ($data['immediate'] == 'no' && (!isset($data['due_date']) || $data['due_date'] == '' || !isset($data['due_time']) || $data['due_time'] == '' || (!isset($data['customer_phone_type']) && !isset($data['customer_physical_type'])) || !isset($data['duration']) || $data['duration'] == ''))) {
			return ['status' => 'fail', 'message' => "Du måste fylla i alla fält"];
		}
	...
	{
		return $consumer_type == 'rwsconsumer' ? 'rws' :

    /**
	* @param $data
	* @return mixed
	*/
	public function storeJobEmail($data, $from)
	{
		...
		if (isset($data['address'])) {
			$job->fill([
				'address' => $data['address'] ?: optional($job->user)->userMeta->address,
				'instructions' => $data['instructions'] ?: optional($job->user)->userMeta->instructions,
				'town' => $data['town'] ?: optional($job->user)->userMeta->city,
			]);
		}
		...
		$this->mailer->from()->send($recipientEmail, $recipientName, $subject, 'emails.job-created', $send_data);
	...
	*/
	public function jobEnd($post_data = [])
	{
		...
		$translatorJobRel = $job->translatorJobRel->first();
		
		if ($translatorJobRel && !$translatorJobRel->completed_at && !$translatorJobRel->cancel_at) {
			$this->sendSessionEndedEmail($translatorJobRel, $post_data['userid'], 'lön');
			$translatorJobRel->completed_at = $completedDate;
			$translatorJobRel->completed_by = $post_data['userid'];
			$translatorJobRel->save();
			
			$this->fireSessionEndedEvent($job, $translatorJobRel, $post_data['userid']);
		}
	}

	protected function sendEmail($email, $name, $subject, $view, $data)
	{
		$mailer = new AppMailer();
		$mailer->send($email, $name, $subject, $view, $data);
	}

	protected function sendSessionEndedEmail($translatorJobRel, $userId, $forText)
	{
		$user = $translatorJobRel->user;
		$email = $user->email;
		$name = $user->name;
		$subject = 'Information om avslutad tolkning för bokningsnummer # ' . $translatorJobRel->job->id;
		$sessionTime = $translatorJobRel->job->session_time;
		
		$data = [
			'user' => $user,
			'job' => $translatorJobRel->job,
			'session_time' => $sessionTime,
			'for_text' => $forText,
		];
	...
	protected function fireSessionEndedEvent($job, $translatorJobRel, $userId)
	{
